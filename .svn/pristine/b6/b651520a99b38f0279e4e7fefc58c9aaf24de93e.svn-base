...
//    tempImageView.frame = CGRectMake(0, 0, sourceView.width, sourceView.height);
//    tempImageView.image = [self placeholderImageForIndex:self.currentImageIndex];
//    [self.view addSubview:tempImageView];
//    tempImageView.contentMode = UIViewContentModeScaleAspectFit;

    CGFloat placeImageSizeW = 295; //tempImageView.image.size.width;
    CGFloat placeImageSizeH = 166; //tempImageView.image.size.height;
    CGRect targetTemp;
...
- (void)setupImageOfImageViewForIndex:(NSInteger)index
{
    ...
    if ([self highQualityImageURLForIndex:index]) {
        ...
        [view setImageWithURL:[self highQualityImageURLForIndex:index] placeholderImage:[UIImage imageNamed:@"traveldetails_img"]];
    }
