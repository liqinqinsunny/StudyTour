...
- (NSDictionary *)getSubmitParam
{
    ...
    [tempParam setValue:model.stuName forKey:@"initiatorName"];
...
//                [imageButton setBackgroundImage:image forState:UIControlStateNormal];
    ...
    if (_dataModel.content.length > 0 && ![_dataModel.content isEqualToString:@"写点什么吧！"]) {
        ...
        _contentTextView.textColor = [UIColor colorWithHex:0xcccccc];
    }
...
{
    WriteWordsOfTravelNoteViewController *wwtnvc = [[WriteWordsOfTravelNoteViewController alloc] init];
    wwtnvc.dataModel = _dataModel;
    
    if (![textView.text isEqualToString:@"写点什么吧！"]) {
        wwtnvc.contentString = textView.text;
    }
    
    [self.navigationController pushViewController:wwtnvc animated:YES];
    return NO;
}
...
//            ALAssetOrientation ooo = [representation orientation];
            ...
            NSString *lastVar;
            if (weakself.dataModel.imageArray.count > 0) {
                id imageInfo = [weakself.dataModel.imageArray lastObject];
                if ([imageInfo isKindOfClass:[NSString class]]) {
                    NSArray *jp_vars = [imageInfo componentsSeparatedByString:@"_"];
                    NSString *lastPathVar = [jp_vars lastObject];
                    NSInteger currentVar = [lastPathVar integerValue];
                    if (currentVar != weakself.dataModel.imageArray.count-1) {
                        lastVar = [NSString stringWithFormat:@"%ld", currentVar + 1];
                    }
                }
            }
            NSString *lastPath;
            if (lastVar == nil) {
                lastPath = [NSString stringWithFormat:@"WriteTravelNoteImageCache%@_%@_%lu", cardId, _templateId, (unsigned long)(_dataModel.imageArray.count)];
            } else {
                lastPath = [NSString stringWithFormat:@"WriteTravelNoteImageCache%@_%@_%@", cardId, _templateId, lastVar];
            }
...
- (void)compressImageWithImage:(UIImage *)image WriteTo:(NSString *)filePath Last:(BOOL)isLast
{
    ...
    CGFloat imagerate = [self compressCoefficient:data.length];
...
- (CGFloat)compressCoefficient:(NSInteger)dataSize
{
    CGFloat imagerate = 0.5f;
    if (dataSize < 200 * 1024) {
        imagerate = 1.0f;
    } else if (dataSize < 500 * 1024) {
        imagerate = 0.9f;//可以  600k -> 200k
    } else if (dataSize < 1000 * 1024) {
        imagerate = 0.8f;//可以  600k -> 200k
    } else if (dataSize < 3000 * 1024) {
        imagerate = 0.7f;
    } else if (dataSize < 5000 * 1024) {
        imagerate = 0.5f;// 4M -> 240k
    } else if (dataSize < 10000 * 1024) {
        imagerate = 0.3f;// 5-6M -> 300k+
    } else {
