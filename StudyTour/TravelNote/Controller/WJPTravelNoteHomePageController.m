...
- (void)loadTravelNoteDataFromServer
{
    ...
    [downloader POST:AllGeneralize cachePath:cachefilePath param:param wating:^{
        ...
        // 如果缓存数据和现在获取的数据一样的就返回[1]	(null)	@"cardId" : @"330727200011110012"
...
- (void)updateFeedBackToServerWithStatus:(NSInteger)status incidentId:(NSString *)incidentId inSection:(NSInteger)section
{
    ...
    Downloader *downloader = [[Downloader alloc] init];
    NSMutableDictionary *m_param = [NSMutableDictionary new];
    [m_param setValue:incidentId forKey:@"incidentId"];
    [m_param setValue:userInfo.stuName forKey:@"stuName"];
    [m_param setValue:userInfo.cardId forKey:@"cardId"];
    [m_param setValue:userInfo.groupId forKey:@"groupId"];
    [m_param setValue:feedbackStatus forKey:@"total"];
    NSDictionary *param = [m_param copy];
